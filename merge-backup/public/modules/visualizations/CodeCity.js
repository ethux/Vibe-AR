...
//  Routed through proxy (/api/codecity/analyze)
// ═══════════════════════════════════════════════════════════════════
...
class CodeCityRenderer {
  ...
  async analyzeCode(code, language, filename) {
    ...
    try {
      ...
      log('[CODECITY] Analysis failed: ' + e.message);
      return null;
    }
  }
  ...
  renderCity(layout) {
    ...
    // Buildings (cubes with matrix rain)
    if (layout.buildings) {
    ...
    if (layout.connections) {
      ...
      for (const conn of layout.connections) {
        ...
        const midPoint = new THREE.Vector3(
          (fromPos.x + toPos.x) / 2, midY, (fromPos.z + toPos.z) / 2
        );
    ...
    if (layout.hotspots && layout.buildings) {
      ...
      for (const hotId of layout.hotspots) {
        ...
        const ringMat = new THREE.MeshBasicMaterial({
          color: 0xEF4444, transparent: true, opacity: 0.7, side: THREE.DoubleSide
        });
  ...
  _showTooltip(entry) {
    ...
    this._tooltipWindow = this.wm.createWindow({
      ...
      width: 0.22, height: 0.12,
      position: tooltipPos,
      content: (ctx, w, h) => {
        const typeColors = {
          'class': '#F97316', 'function': '#3B82F6',
          'variable': '#22C55E', 'import': '#A855F7'
        };
  ...
  onGrabStart(handIdx, grabPoint) {
    ...
    if (other.grabbing) this._initTwoHandAnchor();
  }
  ...
  onGrabMove(handIdx, grabPoint) {
    ...
    if (gs.grabbing && otherGs.grabbing) {
      ...
      const cosR = Math.cos(newRotY), sinR = Math.sin(newRotY);
      const sx = lx0 * newScale, sz = lz0 * newScale;
      this.cityGroup.position.x = wx0 - (sx * cosR + sz * sinR);
      this.cityGroup.position.z = wz0 - (-sx * sinR + sz * cosR);
  ...
  _initTwoHandAnchor() {
    ...
    this._twoHandAnchor = {
      local0: new THREE.Vector3(p0.x, p0.y, p0.z).applyMatrix4(invMatrix),
      local1: new THREE.Vector3(p1.x, p1.y, p1.z).applyMatrix4(invMatrix),
