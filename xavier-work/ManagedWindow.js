...
// ═══════════════════════════════════════════════════════════════════
//
//  Classic Windows 95 look: blue title bar, 3D beveled borders,
//  silver frame, min/max/close buttons in title bar.
//
...
class ManagedWindow {
  ...
  _build(pos) {
    ...
    // ── Content area (inset, white/black) ──
    this._buildContent(W, H, BORDER, TITLEBAR_H);

    // ── Title bar (blue gradient with text + buttons) ──
  ...
  _drawContentCanvas() {
    ...
    // Win95 content area: white background with inset 3D border
  ...
  _drawTitleCanvas() {
    ...
    // Blue gradient background (navy → lighter blue, left to right)
    ...
    // Win95 title bar buttons area (right side) — drawn as part of canvas
    // Close button: [X]   Maximize: [□]   Minimize: [_]
    const btnSize = 14;
  ...
  _buildBorders(W, H, border, titleH) {
    this._borderMeshes = {};
    const bevelW = 0.004; // bevel strip width in meters

    // Outer highlight (white on top + left = raised look)
    const makeHighlightMat = () => new THREE.MeshBasicMaterial({
      color: 0xFFFFFF, transparent: true, opacity: 0.9, side: THREE.DoubleSide
    });
    // Outer shadow (dark on bottom + right)
    ...
    const leftMat = makeHighlightMat();
  ...
  _buildResizeHandles(W, H, border, titleH) {
    ...
    // Win95-style diagonal grip lines (bottom-right resize indicator)
    const makeCornerCanvas = () => {
      ...
      // Diagonal grip lines (Win95 style)
      ctx.fillStyle = PixelArt.SHADOW;
      for (let i = 0; i < C; i++) {
        if (i % 3 === 0) {
  ...
  _updateHover(dt) {
    ...
    // Drag bar (title bar) hover — subtle highlight
    const dragHover = this.hoverTarget === 'dragBar';
    // No visual change needed for Win95 title bar drag — it's already prominent
