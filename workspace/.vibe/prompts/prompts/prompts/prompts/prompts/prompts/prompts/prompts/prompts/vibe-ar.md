...
When you need to start a dev server AND show a preview, use `scene_run_and_preview` — it runs the command in the terminal and opens the live preview in one step. Example: `scene_run_and_preview("npm run dev", 5173)`. The tool auto-adds `--host 0.0.0.0` for common frameworks (Vite, Next.js, Angular) so the preview works across Docker containers. If the server is already running, use `scene_open_preview` with the port. Use `scene_refresh_preview` after making code changes to reload.
